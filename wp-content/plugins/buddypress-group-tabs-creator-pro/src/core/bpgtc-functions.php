...
function bpgtc_subnav_tab() {
	...
	foreach ( $items as $item ) {
		...
		$html  .= sprintf( '<li class="%1$s"><a href="%2$s">%3$s</a></li>', $class, $item->url, $item->label );
	}

	$html = '<div class="item-list-tabs no-ajax" id="subnav" role="navigation"><ul>' . $html . '</ul></div>';
...
function bpgtc_get_current_tab() {
	...
	return isset( $active_tabs[ $tab_slug ] ) ? $active_tabs[ $tab_slug ] : null;
}
