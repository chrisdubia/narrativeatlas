...
class Labels extends Model {
	...
	const TEXT_DOMAIN = 'cm-maps-routes-custom-fields-addon';
	...
	public static function init() {
		...
		add_action('cmmrmcf_load_label_file', array(__CLASS__, 'loadLabelFile'), 1);
		
		static::loadLabelFile();
		do_action('cmmrmcf_labels_init');
		
		/* You can use the following filters to add new labels for plugin:
		add_filter('cmmrmcf_labels_init_labels', function($labels) {
			$labels['label_name'] = array('default' => 'Value', 'desc' => 'Description', 'category' => 'Other');
			return $labels;
		});
		add_filter('cmmrmcf_labels_init_labels_by_categories', function($labelsByCategories) {
			$labelsByCategories['Other'][] = 'label_name';
			return $labelsByCategories;
		});
		*/
		
		static::$labels = apply_filters('cmmrmcf_labels_init_labels', static::$labels);
		static::$labelsByCategories = apply_filters('cmmrmcf_labels_init_labels_by_categories', static::$labelsByCategories);
