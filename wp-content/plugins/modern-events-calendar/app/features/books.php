...
class MEC_feature_books extends MEC_base
{
        ...
        switch ($step)
        {
            ...
            case '2':
                ...
                $validate_pattern_value = function($value, $type, $pattern, $label) use ($prepare_pattern)
                {
                    ...
                    if (function_exists('wp_unslash')) $value = wp_unslash($value);
    ...
    public function tickets_availability()
    {
        ...
        if(is_array($tickets))
        {
            ...
            {
                if(!array_key_exists($tid, $prices)) continue;

                $label = $prices[$tid];
                if(!is_string($label) || trim($label) === '')
                {
                    $numeric = $this->book->get_ticket_price_key($ticket, current_time('Y-m-d'), $event_id, 'price', $date);
                    $label = $this->main->render_price(($numeric ?: 0), $event_id);
                }

                $prices[$tid] = apply_filters('mec_filter_price_label', $label, $ticket, $event_id, $this->book);
            }
        }
    ...
    public function tickets_availability_multiple()
    {
        ...
        $tickets = get_post_meta($event_id, 'mec_tickets', true);
        $ts_for_price = (count($dates) === 1) ? $dates[0] : null;
        if(is_array($tickets))
        {
            foreach($tickets as $tid => $ticket)
            {
                ...
                if(!is_string($label) || trim($label) === '')
                {
                    $numeric = $this->book->get_ticket_price_key($ticket, current_time('Y-m-d'), $event_id, 'price', $ts_for_price);
                    $label = $this->main->render_price(($numeric ?: 0), $event_id);
                }
