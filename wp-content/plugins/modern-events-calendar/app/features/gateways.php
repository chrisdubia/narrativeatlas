...
class MEC_gateway_stripe_connect extends MEC_gateway
{
    ...
    public function authenticate()
    {
        ...
        $called = isset($_GET['mec-stripe-connect']) && sanitize_text_field($_GET['mec-stripe-connect']) == 1;
        if(!$called) return;
        ...
        // Use POST with form-encoded body per Stripe OAuth spec
        $fields = array(
        ...
        $request = wp_remote_post('https://connect.stripe.com/oauth/token', array(
            'timeout' => 20,
            'body' => $fields
        ));
        ...
        $stripe_user_id = $response->stripe_user_id ?? null;
    ...
    public function express_redirect()
    {
        ...
        $called = isset($_GET['mec-stripe-connect-express-redirect']) && sanitize_text_field($_GET['mec-stripe-connect-express-redirect']) == 1;
        if(!$called) return;
    ...
    public function express_refresh()
    {
        ...
        $called = isset($_GET['mec-stripe-connect-express-refresh']) && sanitize_text_field($_GET['mec-stripe-connect-express-refresh']) == 1;
        if(!$called) return;
    ...
    {
        // Is user returned?
        $called = isset($_GET['mec-stripe-connect-express-return']) && sanitize_text_field($_GET['mec-stripe-connect-express-return']) == 1;
        if(!$called) return;
    ...
    public function express_dashboard()
    {
        ...
        // Is it a request to redirect?
        $called = isset($_GET['mec-stripe-connect-express-dashboard']) && sanitize_text_field($_GET['mec-stripe-connect-express-dashboard']) == 1;
        if(!$called) return;
        ...
        {
            // Do not silently delete connection; show reconnect prompt in settings UI instead
            return;
        }
    }
    ...
    public function get_fee_amount($transaction)
    {
        ...
        $fee_type = $this->options['fee_type'] ?? 'amount';
        $fee_per = $this->options['fee_per'] ?? 'ticket';
    ...
    public function do_transaction($transaction_id = NULL)
    {
        ...
        // Invalid Transaction
        if(!is_array($transaction) || !isset($transaction['price'])) return false;
    ...
    public function op_form($options = array())
    {
        ...
        if($connection_method === 'express')
        {
            ...
            {
                // Do not silently remove saved connection; instead, mark for reconnect prompt
                $connection_validation_failed = true;
            }
        }
        else if($strip_account_id)
        {
            // For standard method, verify that the saved account still exists
            try
            {
                \Stripe\Stripe::setApiKey($this->options['secret_key']);
                \Stripe\Account::retrieve($strip_account_id);
            }
            catch(Exception $e)
            {
                $connection_validation_failed = true;
        ...
        <div class="mec-gateway-options-form">
            ...
            <?php
            // Display platform fee summary for organizers' clarity
            $fee        = isset($this->options['fee']) ? floatval($this->options['fee']) : 0;
            $fee_type   = $this->options['fee_type'] ?? 'amount';
            $fee_per    = $this->options['fee_per'] ?? 'booking';
            $fee_label  = $fee_type === 'percent' ? ($fee . '%') : $fee;
            ?>
            <p class="mec-info">
                <?php echo sprintf(esc_html__('Platform fee: %1$s per %2$s', 'mec'), esc_html($fee_label), esc_html($fee_per)); ?>
            </p>
            ...
            <div class="mec-form-row">
                <div class="mec-col-12">
                    ...
                    <?php if($connection_method === 'express'): ?>
                        <?php if(!empty($connection_validation_failed)): ?>
                            <div class="mec-error" role="alert"><?php esc_html_e('Stripe connection could not be verified. Please reconnect your account.', 'mec'); ?></div>
                        <?php endif; ?>
                        ...
                        <div id="mec_gateway_options_form_stripe_connection_success" class="<?php echo ($strip_account_id && empty($connection_validation_failed) ? '' : 'mec-util-hidden'); ?>">
