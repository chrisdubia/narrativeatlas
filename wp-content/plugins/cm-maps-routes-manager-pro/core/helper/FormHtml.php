...
class FormHtml {
	...
	static function checkboxTreeEdit($fieldName, array $current, array $values, $parentId = 0) {
		$output = '';
		if (!empty($values[$parentId])) {
			$output .= '<ul class="'. App::prefix('-form-checkbox-tree') .'">';
			foreach ($values[$parentId] as $id => $label) {
				$output .= sprintf('<li><label><input type="checkbox" name="%s" value="%s"%s /><span>%s</span></label><span class="plus dashicons dashicons-plus-alt"></span>%s</li>',
					$fieldName,
					esc_attr($id),
					checked(in_array($id, $current), true, false),
					esc_html($label),
					static::checkboxTreeEdit($fieldName, $current, $values, $id)
				);
			}
			$output .= '</ul>';
		}
